...
class BenchmarkRunner:
    ...
    def run_benchmark(self, evtx_folder=None, output_file=None):
        ...
        # Prevent CSV/Formula Injection by sanitizing string columns
        for col in df_results.select_dtypes(include=['object', 'string']).columns:
            df_results[col] = df_results[col].apply(lambda x: f"'{x}" if isinstance(x, str) and x.startswith(('=', '+', '-', '@')) else x)
