...
## 2024-04-26 - Prompt Injection in LLM Analysis
**Vulnerability:** The application was vulnerable to prompt injection because untrusted event logs were passed to the LLM directly alongside system instructions, allowing malicious log content to override LLM behavior.
**Learning:** Untrusted inputs (logs, concepts, retrieved context) must be clearly separated from instructions when passed to an LLM.
**Prevention:** Wrap untrusted inputs in XML-style tags (e.g., `<log_data>`) within the user role, and explicitly instruct the LLM in the system prompt to treat content within these tags strictly as data and ignore any instructions found within them.
